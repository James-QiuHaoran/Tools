...
Author: Haoran Qiu
Adapted from: https://github.com/pmav99/nvsmi/blob/master/nvsmi.py
...
NVIDIA_SMI_GET_GPUS = "nvidia-smi --query-gpu=index,uuid,utilization.gpu,memory.total,memory.used,memory.free,driver_version,name,gpu_serial,display_active,display_mode,temperature.gpu,clocks.max.sm,clocks.current.sm,power.draw --format=csv,noheader,nounits"
...
def _get_gpu(line):
    ...
    max_freq_mhz = to_float_or_inf(values[12])
    curr_freq_mhz = to_float_or_inf(values[13])
    power_watts = to_float_or_inf(values[14])
    gpu = GPU(
        id,
        uuid,
        gpu_util,
